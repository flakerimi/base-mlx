...
use crate::tokenizer::Utf8TokenDecoder;
...
pub fn check_compatible(target: &LoadedModel, draft: &LoadedModel) -> Result<()> {
    ...
    if target.cfg.model_type != draft.cfg.model_type {
        return Err(Error::ModelLoad(format!(
            "draft/target architecture mismatch: target={} draft={}",
            target.cfg.model_type, draft.cfg.model_type
        )));
    }
    Ok(())
}
...
pub fn generate<F: FnMut(&str, u32)>(
    ...
    let mut target_cache = target.model.new_cache();
    let mut draft_cache = draft.model.new_cache();
    let prompt_len = prompt_tokens.len();

    // ── Prefill ──
    let target_prefill = target
        .model
        .forward_multi(prompt_tokens, &mut target_cache)?;
    ...
    emitted.push(leader);
    decoder.push(&target.tokenizer, leader, &mut on_token)?;
    ...
    'outer: while (emitted.len() as u32) < max_new_tokens {
        ...
        let target_logits = target
            .model
            .forward_multi(&verify_input, &mut target_cache)?;
        ...
        for &t in &drafts[..n_accepted] {
            ...
            emitted.push(t);
            decoder.push(&target.tokenizer, t, &mut on_token)?;
        }
        if (emitted.len() as u32) >= max_new_tokens {
            break 'outer;
        }
        ...
        emitted.push(bonus);
        decoder.push(&target.tokenizer, bonus, &mut on_token)?;
    ...
    let text = decoder.finish(&target.tokenizer, &mut on_token)?;
